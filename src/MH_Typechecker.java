...
class MH_Typechecker {
    ...
    static MH_TYPE computeType (MH_EXP exp, TYPE_ENV env)
			//exp = expressions // env = type environment
	throws TypeError, UnknownVariable {

        // add code here
		if (exp.isBOOLEAN()){
			return BoolType; // Boolean returns boolean type
		}
		if (exp.isNUM()){
			return IntegerType; // Number returns integer type
		}
		if (exp.isVAR()){
			return env.typeOf(exp.value()); // Variable returns an exp expression that will map the variable to the
											// associated type
		}
		if (exp.isAPP()){ // application expression e1,e2 . The type of e2 should match the argument
							// type of what e1 expects (e.g. if (e1(type1)), e2's type should be type1)
							// and the return type should be the type of e1(e2).
			if ((computeType(exp.first(),env).left().equals(computeType(exp.second(),env)))){
				return computeType(exp.first(),env).right();
			}else{
				throw new TypeError ("Application operation invalid, type mismatch");
			}
		}
		if (exp.isINFIX()){ // infix = '==''<=''+''-'
			//Since that in Hint no 2 on page 9 states the following:
			// Assume that it should only accept integer arguments
			// But depends on the infix operation in question (string comparison uses '=='
			//hence the starting condition will be needed to see if the exp1 and exp2 is an integer.
			//or that exp1 equals to exp2
			if ((!computeType(exp.first(),env).isInteger() || (!computeType(exp.second(),env).isInteger())||
					!(computeType(exp.first(),env).equals(computeType(exp.second(),env))))){
				throw new TypeError("Integer types are needed for infix operations.");
			}else{
				switch (exp.infixOp()){
					...
					case "-" : return IntegerType;
					default : throw new TypeError("Infix operator provided are not supported");
				}
			}
		}
		if (exp.isIF()){
			//If exp1 then exp2 else exp3
			//exp1 is required to be a boolean statement (use isBool)
			//exp2 and exp3 must have the same type;
			if (!computeType(exp.first(),env).isBool()){
				throw new TypeError("Expression 1's type is: " + computeType(exp.first(),env) +
						", boolean type statement is expected.");
			}else if (!computeType(exp.second(),env).equals(computeType(exp.third(),env))){
				throw new TypeError("Expression 2's type and Expression 3's type is not a " +
						"match.");
			}else if (computeType(exp.second(),env).equals(computeType(exp.third(),env))){
				return computeType(exp.second(),env);
			}else{
				throw new TypeError("Expression invalid");
			}
		}
		if (exp.isLAMBDA() || exp.isREF()){
			throw new TypeError("Expression unsupported"); // told from instructions that it should be ignored
		}
		throw new TypeError("Expression unsupported");
